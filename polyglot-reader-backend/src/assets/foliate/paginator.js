...
export class Paginator extends HTMLElement {
    ...
    #replaceBackground(background, columnCount) {
        ...
        const themeBgColorRaw = htmlStyle.getPropertyValue('--theme-bg-color')
        const themeBgColor = (themeBgColorRaw || '').trim() || htmlStyle.backgroundColor || '#ffffff'

        const textureImageRaw = htmlStyle.getPropertyValue('--pg-bg-texture-image')
        const textureImage = (textureImageRaw || '').trim()
        const opacityRaw = htmlStyle.getPropertyValue('--pg-bg-texture-opacity')
        const opacityNum = parseFloat((opacityRaw || '').trim())
        const textureOpacity = Number.isFinite(opacityNum) ? Math.max(0, Math.min(1, opacityNum)) : 0
        const sizeRaw = htmlStyle.getPropertyValue('--pg-bg-texture-size')
        const textureSize = (sizeRaw || '').trim() || 'cover'
        const repeatRaw = htmlStyle.getPropertyValue('--pg-bg-texture-repeat')
        const textureRepeat = (repeatRaw || '').trim() || 'repeat'
        const blendRaw = htmlStyle.getPropertyValue('--pg-bg-texture-blend-mode')
        const textureBlend = (blendRaw || '').trim() || 'multiply'
        const hasTexture = textureImage && textureImage !== 'none' && textureOpacity > 0

        // Keep legacy adjustments for fixed backgrounds from book CSS, but ensure we never crash on null.
        ...
        for (let i = 0; i < columnCount; i++) {
            ...
            column.style.background = themeBgColor
            if (hasTexture) {
                const layer = document.createElement('div')
                layer.style.position = 'absolute'
                layer.style.inset = '0'
                layer.style.pointerEvents = 'none'
                layer.style.backgroundImage = textureImage
                layer.style.backgroundRepeat = textureRepeat
                layer.style.backgroundSize = textureSize
                layer.style.backgroundPosition = 'top'
                layer.style.mixBlendMode = textureBlend
                layer.style.opacity = `${textureOpacity}`
    ...
    #beforeRender({ vertical, rtl, background }) {
        ...
        if (flow === 'scrolled') {
            ...
            // Ensure background layer is painted in scrolled mode too.
            this.columnCount = 1
    ...
    setStyles(styles) {
        ...
        // Recompute docBackground so "gutter" background updates when styles (e.g. theme/texture) change.
        requestAnimationFrame(() => {
            try {
                const doc = this.#view?.document
